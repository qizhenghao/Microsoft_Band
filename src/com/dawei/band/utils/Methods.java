...
public class Methods {
    ...
    public static int getScreenWidth() {
        if (Variables.screenWidth == 0) {
            WindowManager wm = (WindowManager) BandApplication.getContext().getSystemService(Context.WINDOW_SERVICE);
    ...
    public static void showToast(final CharSequence text, final boolean lengthLong) {
        Runnable update = new Runnable() {
            public void run() {
                if (Variables.gToast == null && BandApplication.getContext() != null) {
                    Variables.gToast = Toast.makeText(BandApplication.getContext().getApplicationContext(), "",
                            Toast.LENGTH_LONG);
                }
        ...
        BandApplication.getApplicationHandler().post(update);
    }
